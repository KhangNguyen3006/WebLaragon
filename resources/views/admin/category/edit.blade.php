...
@method('PUT')
	...
	@csrf
            ...
            <div class="form-group">
                <label for="parentId">ParentID</label>
                 <select name=parentId>
                    ...
                    <option value='{{$cat->id}}' <?php if($cat->id==$category->parentId) echo"selected" ?>>{{$cat->catName}}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class=editor name=description cols=100 rows=5>{{$category->description}}</textarea>
            </div>
